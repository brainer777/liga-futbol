...
import { ArrowLeft, ListChecks, Target, AlertTriangle, BarChart3 } from 'lucide-react';
...
export default function EstadisticasPublicasPage() {
  ...
  const resumen = useQuery<Resumen>({
    queryKey: ['publico', 'estadisticas', 'resumen'],
    queryFn: () => api.get('/publico/estadisticas/resumen').then((r) => r.data),
  });
  const equipos = useQuery<FilaEquipo[]>({
    queryKey: ['publico', 'estadisticas', 'equipos'],
    queryFn: () => api.get('/publico/estadisticas/equipos').then((r) => r.data),
  });
  const goleadores = useQuery<Goleador[]>({
    queryKey: ['publico', 'estadisticas', 'goleadores'],
    queryFn: () => api.get('/publico/estadisticas/goleadores').then((r) => r.data),
  });
  const tarjetas = useQuery<Tarjeta[]>({
    queryKey: ['publico', 'estadisticas', 'tarjetas'],
    queryFn: () => api.get('/publico/estadisticas/tarjetas').then((r) => r.data),
  });
